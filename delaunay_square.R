...
p <- ggplot(data = data, aes(x = x, y = y)) +
  geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = voronoi$delsgs, color = "black", lineend = "round") + 
  #geom_segment(aes(x = x1, y = y1, xend = x2, yend = y2), data = voronoi$dirsgs, color = "red", lineend = "round", alpha = 0.5) + 
...
ggsave("delaunay-square.png", p, width = 24, height = 24, units = "in", dpi = 300)
